...
@Component
public class ExecutiveApprovalStep implements ApprovalStep {
    private static final Logger logger = LoggerFactory.getLogger(ExecutiveApprovalStep.class);
    private final ApprovalMapper approvalMapper;

    @Autowired
    public ExecutiveApprovalStep(ApprovalMapper approvalMapper) {
        this.approvalMapper = approvalMapper;
    }

    @Override
    public void execute(Approval approval) {
        saveStepStatus(approval.getId(), "EXECUTIVE", "COMPLETED");
        approval.setStatus("EXECUTIVE_COMPLETED");
        approvalMapper.updateApprovalStatus(approval.getId(), approval.getStatus());
        logger.info("Approval status updated to EXECUTIVE_COMPLETED for approval: {}", approval.getId());
    }
